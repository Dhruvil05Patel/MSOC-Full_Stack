...
function StylistPage() {
  ...
  const femaleStylists = stylists.filter(stylist => stylist.gender === 'female')
  ...
  const getGenderColor = (gender) => {
    return gender === 'male' ? 'bg-blue-100 text-blue-600' : 'bg-purple-100 text-purple-600'
  }
    ...
    <PageWrapper>
      <div className="py-10 md:py-16 px-4 md:px-8 max-w-7xl mx-auto">
          ...
          >
            {filteredStylists.map((stylist, index) => (
              ...
              >
                ...
                <h3 className="text-lg md:text-xl font-bold mb-2">{stylist.name}</h3>
                <p className="text-gray-600 mb-3">{stylist.specialty || 'Hair & Beauty Expert'}</p>
                <div className="flex items-center justify-center mb-3">
                  <span className="text-yellow-500 mr-1">⭐</span>
                  <span className="text-gray-700 font-semibold">{stylist.rating || 'New'}</span>
                </div>
                <div className="flex items-center justify-center mb-3">
                  <span className={`px-3 py-1 rounded-full text-xs font-medium ${getGenderColor(stylist.gender)}`}>
                    {stylist.gender === 'male' ? 'Male Stylist' : 'Female Stylist'}
                  </span>
                </div>
                {stylist.experience && (
                  <p className="text-sm text-gray-500 mb-2">{stylist.experience} years experience</p>
                )}
                {stylist.branch?.name && (
                  <p className="text-sm text-pink-600 font-medium">{stylist.branch.name}</p>
                )}
              </motion.div>
            ))}
          </motion.div>
            ...
            >
              {maleStylists.map((stylist, index) => (
                ...
                >
                  ...
                  {stylist.branch?.name && (
                    <p className="text-sm text-blue-600 font-medium">{stylist.branch.name}</p>
                  )}
                </motion.div>
              ))}
            </motion.div>
            ...
            >
              {femaleStylists.map((stylist, index) => (
                ...
                >
                  ...
                  {stylist.branch?.name && (
                    <p className="text-sm text-purple-600 font-medium">{stylist.branch.name}</p>
                  )}
                </motion.div>
              ))}
            </motion.div>
        ...
        {filteredStylists.length === 0 && (
          ...
          >
            <p className="text-gray-500 text-lg">No stylists found for this category.</p>
          </motion.div>
        )}
